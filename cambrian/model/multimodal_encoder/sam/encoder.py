...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        self.downsamples = nn.Sequential(*downsamplers)

        self.sam_hd = True
        if self.sam_hd:
            self.hd_alpha_downsamplers = nn.Parameter(torch.zeros(1))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.sam_hd:
            ...
            x = x + first_global_feature * self.hd_alpha_downsamplers
...
def create_sam_vit(
    ...
    **kwargs,
    ...
    # download pretrained weights to ~/.cache/torch/hub/checkpoints/
    if pretrained:
        ckpt_path = SAM_MODEL_CONFIG[model_name]["ckpt_url"]
        if ckpt_path is None:
            raise ValueError(f"Pretrained weights unspecified for {model_name}")
        state_dict = torch.hub.load_state_dict_from_url(ckpt_path)
        image_encoder.load_state_dict(state_dict, strict=False)
        print(f"SAM-ViT model loaded from {ckpt_path}")
